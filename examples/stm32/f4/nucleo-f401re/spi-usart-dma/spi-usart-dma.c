...
void dma1_usart2_setup(DmaDataType* data, size_t size)
{
	...
	dma_channel_select(DMA1, DMA_DEMO_STREAM, DMA_DEMO_CHANNEL);
...
{	
    ...
    dma_set_memory_address(DMA2, DMA_STREAM0, (uint32_t)rx);
    ...
    dma_set_memory_address(DMA2, DMA_STREAM3, (uint32_t)tx);
